...
def clicked():
    ...
    datos=(edadMinima,edadMaxima,genero,sexualidad,colorOjos,colorPiel)
    ...
    sql = "SELECT * FROM personas WHERE edad BETWEEN ? AND ? AND genero=? AND sexualidad=? AND color_ojos=? AND color_piel=?" 
    ...
    if(consulta.execute(sql,datos)):
        ...
        if len(filas)>0:
            ...
            for fila in filas:
                ...
                print("Felicidades, " + str(res) + ". Hiciste match con alguien!!")
                print(fila[1])
            ...
            print("Lo siento, " + str(res) + ", no haces match con nadie.  ")
